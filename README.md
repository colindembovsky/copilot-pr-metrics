...
Besides downloading the Enterprise usage summary for the past 28 days, the script also visualizes PR metrics for Copilot Coding Agent (CCA) and Copilot Code Review (CCR), including created/reviewed/merged throughput, review suggestion acceptance, and median time-to-merge for overall and Copilot-authored pull requests.
...
### Process an already-downloaded JSON file (no API download)

```bash
python copilot_metrics.py \
  --input-json <PATH_TO_DOWNLOADED_JSON>
```

Supported input JSON shapes:
- Full payload generated by this script (contains a top-level `reports` array).
- Single downloaded report file (contains top-level `day_totals`).
- Array of report objects.

If `matplotlib` is not installed, the script still processes and writes JSON output, but skips PNG chart generation.
...
- `--input-json`: Optional path to a local JSON file to process without downloading from the API.
